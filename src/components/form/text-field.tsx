...
export function TextField2(props: InputProps) {
    const {
        field: { onChange, onBlur, value },
        fieldState: { isTouched },
        formState: { errors },
    ...
    return (
        <>
            ...
            <div className="relative">
                ...
                <div
                    className={cn(
                        "absolute left-0 m-0 border-none break-all outline-none",
                        {
                            "top-full": hasError, // Position error message below input
                            "text-sm text-red-500": true, // Style error message
                        }
                    )}
                >
                    <ErrorMessage
                        errors={errors}
                        name={name}
                        render={({ message }) => message && <p className="">{message}</p>}
                    />
                </div>
            </div>
            {/* <ErrorMessage
